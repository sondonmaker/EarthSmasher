...
public static class ProFxParticleSpawner
{
        ...
        bool loop = false)
    {
        ...
        go.transform.localScale = Vector3.one * Mathf.Clamp(scale, 0.05f, 0.28f);
        ...
        bool loop = false)
    {
        ...
        return SpawnWorld(prefab, point, normal, scale, lifetime, loop);
    }
    ...
    static float CapFleetImpactScale(StrikeImpactKind kind, float earthRadius, float scale)
    {
        float maxNorm = kind switch
        {
            ...
            StrikeImpactKind.UfoPop => 0.014f,
            _ => 0.022f
        };
        ...
        out float lifetime)
    {
        switch (kind)
        {
            ...
            case StrikeImpactKind.UfoPop:
                ...
                scaleMul = 0.006f + intensity * 0.004f;
                lifetime = 0.65f;
                return;
