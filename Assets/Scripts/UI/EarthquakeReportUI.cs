...
/// <summary>
/// Earthquake Report 모달.
/// </summary>
public class EarthquakeReportUI : MonoBehaviour
{
    public static EarthquakeReportUI Ensure()
    {
        var ui = Object.FindObjectOfType<EarthquakeReportUI>();
        if (ui != null)
            return ui;
        return new GameObject("EarthquakeReportUI").AddComponent<EarthquakeReportUI>();
    }
    ...
    public void Show(EarthquakeReport r)
    {
        report = r;
        visible = true;
    }
    ...
    void DrawGui()
    {
        ...
        GUI.Label(new Rect(panel.x, panel.y + 8, panel.width, 32), "Earthquake Report", titleStyle);

        Rect content = new Rect(panel.x + 18, panel.y + 48, panel.width - 36, panel.height - 70);
        GUILayout.BeginArea(content);

        Stat("Magnitude", $"M {report.magnitude:0.0}");
        Stat("Region", $"{report.regionName} ({report.regionCode})");
        Stat("Epicenter", $"{report.lat:0.0}, {report.lon:0.0}");
        Stat("Death", report.deaths.ToString("#,0"));
        Stat("Injury", report.injuries.ToString("#,0"));
        Stat("Aftershocks", report.aftershocks.ToString());
